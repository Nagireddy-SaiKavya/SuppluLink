...
public class ProductServiceImplJpa implements ProductService {
    ...
    public List<Product> getAllProductByWarehouse(int warehouseId) throws SQLException {
        try {
            // FK-based first
            List<Product> byFk = productRepository.findAllByWarehouse_WarehouseId(warehouseId);
            ...
            for (Product p : all) if (p.getWarehouse().getWarehouseId() == warehouseId) filtered.add(p);
            return filtered;
